...
class cma_es(object): 
    ...
    def info_register(self):
        ...
        if self.is_info_register:
            ...
            self.hist_condition_number[evalcount-_lambda:evalcount] = max(self.e_value)**2 / min(self.e_value)**2
    ...
    def optimize(self):
        ...
        while self.restart_count < self.restart_budget:
            while True:
                ...
                hist_wcm.append(self.wcm.flatten())
                hist_eigen_value.append(self.e_value.flatten())
                tmp = self.e_vector[:, 0] / np.sum(self.e_vector[:, 0] ** 2.0)
                tmp2 = zeros(self.dim)
                tmp2[0] = 1
                angle = 180. * np.arccos(np.inner(tmp, tmp2)) / np.pi
        ...
        return self.xopt, self.fopt, self.evalcount, self.stop_dict, np.array(hist_wcm), \
            np.array(hist_eigen_value), np.array(hist_angle)